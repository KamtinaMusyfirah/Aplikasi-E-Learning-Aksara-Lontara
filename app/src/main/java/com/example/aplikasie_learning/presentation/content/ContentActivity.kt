package com.example.aplikasie_learning.presentation.content

import androidx.appcompat.app.AppCompatActivity
import android.os.Bundle
import androidx.viewpager.widget.ViewPager
import com.example.aplikasie_learning.adapter.PagesAdapter
import com.example.aplikasie_learning.databinding.ActivityContentBinding
...
class ContentActivity : AppCompatActivity() {
    ...
    private fun viewPagerCurrentPosition() {
        contentBinding.vpContent.addOnPageChangeListener(object : ViewPager.OnPageChangeListener{
            override fun onPageScrolled(
                ...
                val textndex = "${currentPosition + 1} / $totalIndex"
                contentBinding.tvIndexContent.text = textndex

                if (currentPosition == 0){
                    contentBinding.btnPrevContent.invisible()
                    contentBinding.btnPrevContent.disabled()
                }else{
                    contentBinding.btnPrevContent.visiable()
    ...
    private fun getDataIntent() {
        if (intent != null){
            ...
            val material = intent.getParcelableExtra<Materials>(EXTRA_MATERIAL)
    ...
    private fun getDataContent(material: Materials) {
        showLoading()
        contentDatabase
